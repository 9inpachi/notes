...
import RouterApp from './router/ReactRouter';
...
function ComponentsBody({ boxes }) {
    return (
        <div>
            {boxes.map(function (boxData, i) {
                return <Box key={i} image={boxData.image} title={boxData.title} desc={boxData.desc} />;
            })}
            <div style={{ clear: 'both' }}></div>
            <p><ColorBox /></p>
            <p><Button title='Click' href='#' /></p>
        </div>
    );
}

ReactDOM.render(<ComponentsBody boxes={boxes} />, document.getElementById('components'));
ReactDOM.render(<LifeCycle allowChange={true} />, document.getElementById('component-life-cycle'));
ReactDOM.render(<HooksComponent />, document.getElementById('hooks'));
